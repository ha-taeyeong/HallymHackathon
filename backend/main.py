...
CLIENT_REDIRECT_URI = os.getenv("GOOGLE_REDIRECT_URI", "http://localhost:8000/auth/callback")
# CLIENT_REDIRECT_URI = os.getenv("GOOGLE_REDIRECT_URI", "https://planup.p-e.kr/auth/callback")
...
# --------------------
# 정적파일 경로 마운트 (경로 변수 통일)
# --------------------
app = FastAPI()

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
# [중요] 모든 경로는 frontend_dist_path 하나를 기준으로 파생시킵니다.
frontend_dist_path = os.path.normpath(os.path.join(BASE_DIR, "..", "frontend", "dist"))

assets_path = os.path.join(frontend_dist_path, "assets")
images_path = os.path.join(frontend_dist_path, "images") 
index_path = os.path.join(frontend_dist_path, "index.html") # 여기서 정의 끝!
static_path = os.path.join(frontend_dist_path, "static")

# 정적 파일 마운트 (폴더가 존재할 때만 실행)
...
def split_schedule_parts(text: str):
    ...
    # [핵심 수정] 장소명 앞에 붙은 불필요한 '에' 조사 제거
    if place_part.startswith('에'):
        place_part = place_part[1:].strip()
...
# --------------------
# 기본 페이지 및 인증 엔드포인트 (Refactored)
# --------------------

# 1. 메인 페이지 및 React 라우팅 대응
@app.get("/")
@app.get("/memo")  # /memo로 접속해도 React index.html을 던져줍니다.
async def serve_react_app():
    """React 앱의 입구인 index.html을 반환합니다."""
    if not os.path.exists(index_path):
        return HTMLResponse(f"<h2>빌드 파일을 찾을 수 없습니다: {index_path}</h2><p>frontend 폴더에서 npm run build를 먼저 실행해주세요.</p>")
...
def auth_callback(code: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Google 토큰 교환 중 서버 오류 발생.")
    
    token_data = response.json()
    if "access_token" in token_data:
        user_tokens['default'] = token_data
        # 인증 완료 후 리액트의 /memo 페이지로 리다이렉트
        return RedirectResponse(url="/memo")
    else:
...
def safe_parse_datetime(time_obj):
    ...
    def replace_relative_dates(text):
        ...
        # [수정] YYYY-MM-DD 포맷으로 강제 변환하여 dateparser가 절대 헷갈리지 않도록 함
        today_str = now.strftime("%Y-%m-%d")
        tomorrow_str = (now + datetime.timedelta(days=1)).strftime("%Y-%m-%d")
        day_after_tomorrow_str = (now + datetime.timedelta(days=2)).strftime("%Y-%m-%d")
        
        text = re.sub(r'오늘', today_str, text)
        text = re.sub(r'내일', tomorrow_str, text)
        text = re.sub(r'모레', day_after_tomorrow_str, text) # 모레 처리 보완
...
# --------------------
# Catch-all 라우터 (맨 마지막 위치)
# --------------------
@app.get("/{catchall:path}")
async def catch_all_routes(catchall: str):
    """API 요청이 아닌 모든 경로는 React가 처리하도록 index.html로 넘깁니다."""
    # 주소창에 /memo#about 등이 남지 않도록 React index.html을 서빙합니다.
